...
async fn query_results(
    ...
    let results_stream = match state.client.fetch_results(search_job_id, params.raw_docs).await {
        Ok(stream) => {
            tracing::info!(
...
async fn space_savings(
    ...
    let datasets = parse_datasets(params.dataset);
    Ok(Json(state.metadata_client.get_space_savings(&datasets).await?))
}
...
async fn ingestion_details(
    ...
    let datasets = parse_datasets(params.dataset);
    Ok(Json(
        state.metadata_client.get_ingestion_details(&datasets).await?,
    ))
}
...
async fn compression_metadata(
    ...
    Ok(Json(state.metadata_client.get_compression_metadata().await?))
}
...
async fn compression_job(
    ...
    let job = state.metadata_client.submit_compression_job(creation).await?;
