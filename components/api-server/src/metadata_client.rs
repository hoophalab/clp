...
/// surface.
#[derive(Clone, Copy, Debug, Deserialize, Eq, PartialEq, Serialize, ToSchema, TryFromPrimitive, IntoPrimitive)]
#[repr(i32)]
...
impl MetadataClient {
    ...
    pub async fn get_timestamp_column_names(
        ...
        let table_name = self.database().column_metadata_table_name(Some(dataset_name));
        let names: Vec<String> =
            sqlx::query_scalar(&format!("SELECT name FROM `{table_name}` WHERE type IN (?, ?)"))
                .bind(TIMESTAMP_TYPE)
    ...
    async fn submit_and_wait_extract_job(
        ...
        let result = sqlx::query(
            "INSERT INTO query_jobs (job_config, type) VALUES (?, ?)",
        )
        .bind(encoded)
        .bind(job_type_i32)
        .execute(&self.sql_pool)
