...
/// * Returns an error if the default AWS SDK credential provider chain cannot resolve credentials.
...
pub(super) fn compress(
    ...
    let runtime = runtime();
    let input_region = input_source
        .region_code
        .as_ref()
        .map_or(AWS_DEFAULT_REGION, NonEmptyString::as_str);
    let credential_env =
        s3_credential_env(&runtime, input_region, &input_source.aws_authentication)?;
...
/// # Errors
///
/// Returns an error if the default AWS SDK credential provider chain has no provider or fails to
/// resolve credentials.
